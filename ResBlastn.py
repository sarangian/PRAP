...
class ResBlastn:
	...
	def makedb(self):
		...
		db_name = self.install_dir+"databases/res_nucl"
		#create blast databases into the blast+ directory
		makeblastdb = self.blast_dir+"makeblastdb"
		os.system(makeblastdb+" -in "+db_name+" -out res_nucl -dbtype nucl")
	...
	def blastn(self):
		...
		def os_blast(in_name,query_name,output_name):
			...
			os.system(blastn+" -query "+query_name+" -db res_nucl -out "+output_name+\
			...
			for line in blast_content:
				...
				if perc >= self.query_coverage_n and int(items[5]) <= 0:
					...
					if items[0] not in scaf.keys():
						...
						write_content[gene_num] = gene_num+","+items[1]+","+items[2]+\
						","+items[10]+",/,/,"+ar_gene_allele+\
						...
						if contain:
							...
							gene_count += 1
							gene_num = each_name+"_gene_"+str(gene_count)+"_from_"+items[0]
							scaf[items[0]] += [gene_num]
							gene_identity[gene_num] = float(items[2])
							gene_perc[gene_num] = perc
							start_end[gene_num] = [start,end]

...
class Matrix:
	...
	def matrix_file(self):
		...
		pheno_plots = fh.csv_reader(self.install_dir+"/databases/ar_phenotype.csv")
	...
	def matrix_picture(self):
		#to simplify the gene name
		#if the length of the gene name is longer than 15
		#extract the main name and add "*" to represent a modified gene name
		def simplify(genelist):
			simplified_genelist = []
			#get all the big letters
			big_letters = [chr(i) for i in range(65, 91)]
			for i in range(len(genelist)):
				if len(genelist[i]) > 18:
					#split the gene name and find the main name
					content = genelist[i].split("_")
					for word in content:
						if len(word) <= 7:
							letters = list(word)
							for letter in letters:
								if letter in big_letters:
									simplified_genelist.append(word+"*")
									break
				else:
					simplified_genelist.append(genelist[i])
			return simplified_genelist
		...
		for drug_name in self.pheno:
			try:
				...
				if len(ar_gene) == 0:
					...
					df = pd.DataFrame(value,index=genome_id, columns=ar_gene_simp)
		...
		if self.pheno_choose:
			for drug_name in self.pheno:
				try:
					...
					value = {}
					with open(self.install_dir+"/databases/ar_phenotype.csv","r") as csvfile:
					...
					print(contribution[0][0],str(round(contribution[0][1]*100,2))+"%")
...
def main(install_directory,csvfile_directory):
	...
	if os.path.exists(mat.install_dir+"/databases/ar_phenotype.csv"):
		mat.matrix_file()
		...
		print("ar_phenotype.csv not found, skip matrix analyze...")

...
class Blastp:
	...
	#including software install directory, cds files directory
	...
	def __init__(self,install_directory,cds_directory):
		...
		self.cds_file_dir = fh.inpd_get(cds_directory,file_categories="cds files")
		#get all the cds file names form cds_directory
		self.fasta_files = fh.filename_get(self.cds_file_dir,".faa")
		...
		self.blast_files = fh.filename_get(self.cds_file_dir,"ar_prot.xls")
	...
	def blankdel(self):
		for each_name in self.fasta_files:
			content = []
			each = self.cds_file_dir+each_name+".faa"
	...
	def makedb(self):
		...
		db_name = self.install_dir+"databases/ar_prot"
		#create blast databases into the blast+ directory
		makeblastdb = self.blast_dir+"makeblastdb"
		os.system(makeblastdb+" -in "+db_name+" -out ar_prot -dbtype prot")
	...
	def blastp(self):
		...
		def os_blast(in_name,query_name,output_name):
			...
			#add the directory of blastp
			blastp = self.blast_dir+"blastp"
			#run blastp in the terminal with parameters according to blast+ manual
			os.system(blastp+" -query "+query_name+" -db ar_prot -out "+output_name+\
		...
		if len(self.blast_files) == len(self.fasta_files):
			...
			for each_name in self.fasta_files:
				#get the cds file name of the genome
				each = self.cds_file_dir+each_name+".faa"
				#get the output file name of blast
				out_name = self.cds_file_dir+each_name+"VSar_prot.xls"
...
class ParseBlastpResult:
	...
	def __init__(self,install_directory,blast_file_directory):
		...
		self.blast_files = fh.filename_get(self.blast_file_dir,"ar_prot.xls",showext=True)
	...
	def parse(self):
		...
		def annotation(blast_file_name):
			...
			def prot_arinfo_get(ar_info_list):
				str_ar_gene = ""
				str_drug_class = ""
				str_ar_mechan = ""
				ar_gene_allele_r = []
			...
			for line in blast_content:
				...
				ar_gene_allele,drug_class,ar_mechanism = prot_arinfo_get(ar_info)

				#only be annotated when higher than the threshold of identity and coverage
				...
				if float(items[2]) > self.identity_p and perc > self.query_coverage_p:
					if items[0] not in gene_identity.keys():
						...
						#add related information which will be written into a write content list
						write_content[items[0]] = items[0]+","+ar_info[3]+","+items[2]+","+items[10]+","\
						+ar_info[2]+","+ar_info[1]+","+ar_gene_allele+","+drug_class+","+ar_mechanism+"\n"
					#if another similar sequence exceed the indentity, replace the former one
					else:
	...
	#all the protein and nucleotide sequences identified in the annotation files are
	#extracted from old fasta files, and sequences are written into new fasta files in a new directory
	def ar_write(self):
		#extract ar gene sequences from annotated csvfiles and old cds files in the given extension
		def each_write(csvfilename,oldextension,newextension):
			ar_genes = {}
			#get the old cds file name of genome
			old_name = self.blast_file_dir+genome_id+oldextension
		...
		fna_count = 0
		for filename in os.listdir(self.blast_file_dir):
...
def main(install_directory,cds_directory):
	blastp = Blastp(install_directory,cds_directory)
	blastp.blankdel()
	blastp.makedb()
	blastp.blastp()
	pbr = ParseBlastpResult(blastp.install_dir,blastp.cds_file_dir)
	pbr.parse()
	pbr.ar_write()
	return blastp.cds_file_dir

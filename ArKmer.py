...
class Kmer:
	...
	def KmerDB(self):
		...
		def AlleleGet():
			plots = fh.csv_reader(self.install_dir+"databases/ar_parse")
			for row in plots:
				self.ar_allele[row[0]] = row[-1]
				self.drug_class[row[0]] = row[4]
				self.mech_class[row[0]] = row[5]
		...
		def DBget(db_name):
			...
			with open(db_name,"r") as f:
				...
				for i in range(len(indices)-1):
					seq_name = content[indices[i]].split(">")[1].split(" [")[0]
	...
	def KmerReads(self):
							...
							for i in range(len(sequence)-self.k+1):
								if sequence[i:i+self.k] in self.db_kmer:
									for gene in self.db_kmer[sequence[i:i+self.k]]:
										...
										[gene[0].split("|")[-2]]][gene[0]][gene[1]] += 1
						marker = 0
					if "@" in line:
						marker = 1
		try:
		...
		for group in self.fastq_group:
			...
			for gene in predicted_gene:
				...
				gene_count += 1
				f_pred.write(gene_num+","+gene.split("|")[-1]+","+str(predicted_gene[gene][1])+","+str(predicted_gene[gene][0])+\
					","+gene.split("|")[-2]+","+gene.split("|")[1]+","+predicted_gene[gene][2]+","+self.drug_class[gene.split("|")[-2]]+\
					","+self.mech_class[gene.split("|")[-2]]+"\n")
			f_pred.close()
